...
tmpx <-
  ...
  right_index = c(1,2,3,7,8,9,10,11),
...
a_vs_j_result <-
  DAM_tbl %>%
  filter(pvalue < 0.05  & abs(log2fc)  > 0.26 & VIP > 1) %>%
  left_join(feature_anno,c("CompoundID" = "variable_id")) %>%
  arrange(desc(abs(log2fc)))
writexl::write_xlsx(a_vs_j_result,"../04.Result/01.Tables/Adult_vs_Juvenal.xlsx")



DAM_tbl %>% filter(CompoundID == "M245T281_NEG")
expmat %>% filter(name == "M245T281_NEG") %>%
  select(order(colnames(expmat)))

expmat3 <- expmat %>% select(-contains("Size_2")) %>%
  pivot_longer(!name,names_to = 'sample',values_to = 'peak') %>%
  mutate(sample = gsub("..$","",sample)) %>%
  group_by(name,sample) %>%
...
ht_cyc_path_1  =
  ...
  Heatmap(.,
          ...
          show_row_names = F,cluster_columns = F,row_km = 2,row_km_repeats = 1000,
...
tmp_id =
readxl::read_xlsx("../04.Result/01.Tables/Adult_vs_Juvenal.xlsx",sheet = 2)
key_metabo <-
  a_vs_j_result2 %>%
  ...
  mutate(n = 1:nrow(.)) %>%
  inner_join(tmp_id) %>%
  select(Compound.name,n)
r_anno = rowAnnotation(
  foo = anno_mark(at = key_metabo %>% pull(n), labels = key_metabo %>% pull(Compound.name)))
...
pdf(file = paste0("../03.Progress/01.Metabolomics/mFuzz/DAM.pdf"),width = 10.224,height = 8.5)
draw(ht_cyc_enrich)
invisible(dev.off())
